...
**正規化した上での完全一致**とする。全角半角・大文字小文字・空白・記号の揺れは
吸収するが、部分一致は取らない。

部分一致を許すと、正解語を含むだけの誤答（正解「富士山」に対する
「富士山ではありません」など）が自動的に正解と判定され、手動正解ボタンの
意味が薄れるため。別解や表記揺れは人が拾う前提にしている。
...
> **このアプリは凍結されている。** コマンドは残しており動く状態は保つが、
> 積極的にメンテナンスはしない。
>
> PoC の関心が「音声での早押し出題が成立するか」から
> 「LLM がどこまで問題文を補完し、正答できるか」へ移ったため、
> 後者は [`llm-poc`](#llm-poc--llm-予測比較) として別に立てた。
>
> なお `web/src/lib/align.ts` と `lab.ts` はこのディレクトリからのコピーであり、
> オンライン版の文字送りは今もこの実装に依拠している。
